...
const peer = "@deerspangle"; // Replace with your target
...
async function main() {
  ...
  while (true) {
      ...
      const response = await controller.collect({ maxWait: 30000 }, async () => {
           await controller.client.sendText(peer, "Hi! Please say something in the next 30 seconds...");
      });

      if (response.count === 0) {
          await controller.client.sendText(peer, "You did not reply :(");
      } else {
          await controller.client.sendText(peer, `You replied with: ${response.getText()}`);
      }
