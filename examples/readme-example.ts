...
async function main() {
  ...
  if (inlineKeyboards.length > 0) {
      ...
      console.log("Clicking the button matching 'Examples'...");
      try {
        const examplesResponse = await kb.click(/.*Examples/);
        
        if (examplesResponse.text?.includes("Examples for contributing")) {
             console.log("Received expected response!");
        }
      } catch (e) {
          console.log("Could not find or click button (bot might be down or changed).");
      }
  }

  console.log("So what happens when we send an invalid query or the peer fails to respond?");
