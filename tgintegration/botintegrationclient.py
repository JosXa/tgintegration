...
class BotIntegrationClient(InteractionClient):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            session_name=session_name, global_action_delay=global_action_delay, **kwargs
        )
    ...
    def _get_command_list(self) -> List[BotCommand]:
        return self.send(GetFullUser(id=self.peer)).bot_info.commands

    def clear_chat(self) -> None:
        self.send(DeleteHistory(peer=self.peer, max_id=0, just_clear=False))
